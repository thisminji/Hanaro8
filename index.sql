...
-- 1) SHOW, VARIABLES, LIKE 사이 공백 OK
-- 2) innodb_ft% 는 작은따옴표 ' 로 감싸기
-- 3) 마지막에 세미콜론 ;
...
-- 0 row(s) affected, 1 warning(s): 124 
-- InnoDB rebuilding table to add column FTS_DOC_ID Records: 0  Duplicates: 0  Warnings: 1
...
select * from Notice where contents like '%이순신%';
...
WHERE MATCH(title, contents)
      AGAINST('문신* 무신*' IN BOOLEAN MODE);  

-- ex2) 문신과 무신 중 후기 인물은 제외하시오.
SELECT *
FROM Notice
WHERE MATCH(title, contents)
      AGAINST('문신* 무신* -후기*' IN BOOLEAN MODE);

-- ex3) 문신과 무신 중 중기 인물만 출력하시오.
SELECT *
FROM Notice
WHERE MATCH(title, contents)
      AGAINST('문신* 무신* +중기*' IN BOOLEAN MODE);

select count(*), count(1), sum(case when id%2=1 then 1 else ~0 end) from Emp;

-- 이것는 데이터만 만들어짐 FK PK 반영안함
